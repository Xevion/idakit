...
//!
//! A `#[kernel_test(read_only)]` against the corpus manifest's canonical fixture, so it shares one
//! open database with its read-only neighbours and skips when no corpus is configured. Opens
//! `save = false`.
