...
//!
//! A `#[kernel_test]`, since it creates nodes; they are all `"$ "`-prefixed user nodes and nothing
//! is ever saved, so the fixture is untouched on disk. Skips when no corpus is configured.
//!
//! Every case runs in one session, each owning a distinct node name and killing it on the way out,
