...
/// `FF_NAME` (`bytes.hpp`): the address has an explicit or auto-generated name.
///
/// With [`FF_LABL`] it forms IDA's two-bit name classification. The SDK `#undef`s these at the
/// end of the header, so the values are mirrored here and pinned to IDA's own predicates by an
...
unsafe extern "C" {
    /// Set `address`'s name to `name` under `flags` (`SN_*` bits); returns whether it succeeded.
