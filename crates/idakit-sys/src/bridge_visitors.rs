...
//! owned `name: String`.
//!
//! The sink traits, opaque visitors, and the bridge module itself are generated by
//! `build_support/gen.rs` from the declarative manifest in `build_support/visitor_spec.rs`, so
//! their signatures never drift out of sync across the trait, the forwarding `impl`, and the
//! `extern "Rust"` decl; only the hand-written constructors/drivers below stay in this file.
//!
//! # Panic safety
//!
...
/// where the visitor is handed to [`cfunc_walk_ctree`] alongside a [`CtreeVisitor`] that shares the
/// same builder. Passing `sink` as a raw pointer (not `&mut`) lets the caller derive both from one
/// provenance, so per-callback reborrows never conflict.
///
/// # Safety
