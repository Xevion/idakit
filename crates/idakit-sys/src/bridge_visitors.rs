...
//!
//! Per-call data crosses zero-copy, borrowed only for the duration of that one call. Names cross as
//! `&str` (`rust::Str`) or, where not guaranteed valid UTF-8 (ctree names, string literals), as
//! `&[u8]` (`rust::Slice<const uint8_t>`); the sink decodes with [`String::from_utf8_lossy`],
//! matching the crate's lossy-name convention. Child-handle arrays cross as `&[u32]`; struct members
//! and enum constants cross as `&[MemberInfo]` / `&[EnumConstInfo]`, plain POD shared structs with an
//! owned `name: String`.
//!
