...
void scan_rela(ElfW(Addr) base, const ElfW(Rela) *rela, size_t count,
               const ElfW(Sym) *symtab, const char *strtab,
               const char *symname, void *newfn)
{
...
int patch_cb(struct dl_phdr_info *info, size_t, void *)
{
  ...
  for ( const ElfW(Dyn) *d = dyn; d->d_tag != DT_NULL; d++ )
  {
    switch ( d->d_tag )
    {
      case DT_SYMTAB:   symtab = reinterpret_cast<const ElfW(Sym) *>(fixptr(d->d_un.d_ptr)); break;
      case DT_STRTAB:   strtab = reinterpret_cast<const char *>(fixptr(d->d_un.d_ptr)); break;
      case DT_JMPREL:   jmprel = reinterpret_cast<const ElfW(Rela) *>(fixptr(d->d_un.d_ptr)); break;
      case DT_PLTRELSZ: pltrelsz = d->d_un.d_val; break;
      case DT_RELA:     rela = reinterpret_cast<const ElfW(Rela) *>(fixptr(d->d_un.d_ptr)); break;
...
T guarded(T trapval, bool capture, F &&fn)
{
  install_exit_trap();
  g_trapped = false;
...
{
  return guarded<int>(IDAKIT_EXIT_TRAPPED, false, [&] { close_database(save != 0); return 0; });
}

extern "C" int idakit_last_exit_code(void)
{
  return g_exit_code;
}

extern "C" int idakit_was_trapped(void)
{
  return g_trapped ? 1 : 0;
}
...
static uint8_t reg_class_for_optype(uint8_t t)
{
  switch ( t )
  {
    case o_trreg:  return RC_TEST;
    case o_dbreg:  return RC_DEBUG;
    case o_crreg:  return RC_CONTROL;
    case o_fpreg:  return RC_ST;
    case o_mmxreg: return RC_MMX;
    case o_xmmreg: return RC_XMM;
    case o_ymmreg: return RC_YMM;
    case o_zmmreg: return RC_ZMM;
    case o_kreg:   return RC_MASK;
    default:       return RC_GPR;
  }
}
...
static int addr_width(const insn_t &insn)
{
  return ad64(insn) ? 8 : (ad32(insn) ? 4 : 2);
}
...
extern "C" int idakit_decode_insn(idakit_ea_t ea, idakit_insn_t *out)
{
  try
  {
    ...
    uint8_t flow = 0;
    if ( call )     flow |= IDAKIT_FLOW_CALL;
    if ( ret )      flow |= IDAKIT_FLOW_RET;
    if ( is_jump )  flow |= IDAKIT_FLOW_JUMP;
    if ( indirect ) flow |= IDAKIT_FLOW_INDIRECT;
    if ( stops )    flow |= IDAKIT_FLOW_STOPS;
    out->flow = flow;
