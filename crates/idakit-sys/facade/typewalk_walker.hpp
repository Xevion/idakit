...
// (namespace idakit_cxx). The walker is a faithful parallel of the raw facade's type_walker_t: the
// same recursion and the same recursion-safe placeholder + `defined`-set dedup, emitting through
// the cxx opaque visitor's member functions. Its full definition lives in typewalk_cxx.cc, which is
// compiled in the cxx bridge (with the generated visitor header on its include path); the ctree
// walk (hexrays.cpp) is a plain facade TU without that path, so it drives the walker only through
// this opaque handle. One walker per type source (a named type, a prototype, a whole ctree), so
// shared named types dedup across it.
