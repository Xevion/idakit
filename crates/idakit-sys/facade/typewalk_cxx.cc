// cxx extern "Rust" opaque-visitor type walk (namespace idakit_cxx). visit_walker_t is a faithful
// parallel of the raw facade's type_walker_t (facade/type_walk.cpp): the same depth-first tinfo_t
// recursion and the same recursion-safe placeholder + `defined`-set dedup, kept C++-side. The one
// difference is the emit target: instead of an idakit_type_vtbl_t of C function pointers + a void*
// ctx, it calls the extern "Rust" opaque visitor's member functions (vis.scalar(...),
// vis.named_ref(...), vis.fill_struct(...)) that cxx generates. Per-call names cross as rust::Str
// and arrays as rust::Slice, borrowed for the duration of the one call.
...
// The generated header defines the TypeWalkVisitor class (its member functions) and the
// MemberInfo / EnumConstInfo shared structs; typewalk_cxx.h only forward-declares TypeWalkVisitor.
