// cxx-bridged qvector<T> facade (namespace bridge): the KDAB per-instantiation recipe applied to
// two of IDA's own generic containers. intvec_t (qvector<int>) and rangevec_t (qvector<range_t>)
// are each bound as a cxx Opaque ExternType and read two ways: a copying shim to an owned
// rust::Vec, and a zero-copy rust::Slice borrowed from the container's {array, n} (never touching
// alloc/qalloc/qfree; Rust only reads). The cross-check in roundtrip.rs proves both agree with the
...
// (user-declared ctors) fails. qvector's members are all inline in pro.h, so use-based implicit
// instantiation emits exactly the members we call (push_back/size/begin/empty/ctor/dtor) and none
// of the asserting ones, which is all we need. So no explicit instantiation is used.

// int is 32-bit on every target this crate builds (LP64 / LLP64), so an intvec_t's backing array
// is bit-identical to a [i32]; the zero-copy slice cast below relies on it.
...
// Widen each element to a fixed-width int32_t explicitly, rather than relying on int already
// being 32-bit (see the static_assert above).
...
// Zero-copy: borrow {array, n} straight from the qvector, mirroring intvec_slice above.
